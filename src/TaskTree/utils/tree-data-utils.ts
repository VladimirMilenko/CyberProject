 ...
 *
 * @param tree - TreeData
 * @param searchIndex - TreeIndexOfNode
 *
 */
export function getPathToNodeAtTreeIndex(tree, searchIndex) {
    console.log(searchIndex);
    let currentIndex = 0;
    let path = [];
    for (let blockIndex = 0; blockIndex < tree.length; blockIndex++) {
        let block = tree[blockIndex];
        if (searchIndex == currentIndex) {
            path.push(block);
            return {path: path, insertIndex: blockIndex};
        }
        if (block.children && block.expanded)
            for (let taskIndex = 0; taskIndex < block.children.length; taskIndex++) {
                currentIndex++;

                let task = block.children[taskIndex];
                if (currentIndex == searchIndex) {
                    path.push(block);
                    path.push(task);
                    return {path: path, insertIndex: taskIndex};
                }
                if (task.children && task.expanded) {
                    for (let subTaskIndex = 0; subTaskIndex < task.children.length; subTaskIndex++) {
                        let subTask = task.children[subTaskIndex];
                        if (task.expanded)
                            currentIndex++;
                        if (currentIndex == searchIndex) {
                            path.push(block);
                            path.push(task);
                            path.push(subTask);
                            return {path: path, insertIndex: subTaskIndex};
                        }
                    }
                }
            }
...
function addNodeAtDepthAndIndex({
    ...
    if (currentDepth === targetDepth - 1) {
        ...
        let childIndex = currentIndex + 1;
        let insertedTreeIndex:any = null;
